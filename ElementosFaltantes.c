...
void elementosFaltantes(int n, int vetor[], int menor, int maior) {
    quickSort(vetor, 0, n - 1); // Ordenação do vetor inteiro

    int indice = buscaBinariaTeto(0, n - 1, vetor, menor); // Encontrar o índice do menor elemento maior ou igual a 'menor'

    for (int i = menor; i <= maior; i++) {
        if (indice >= n || vetor[indice] > i) { // Elemento não encontrado no vetor
            printf("%d ", i);
        } else {
            // Avançar para o próximo elemento no vetor
            indice++;
        }
    }
}
